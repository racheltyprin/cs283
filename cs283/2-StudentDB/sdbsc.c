...
int del_student(int fd, int id)
{
    ...
    off_t offset = id * sizeof(student_t);
    if (lseek(fd, offset, SEEK_SET) == -1 || write(fd, &empty_student, sizeof(student_t)) != sizeof(student_t)) {
        printf(M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    }
...
int count_db_records(int fd)
{
    ...
    printf(count ? M_DB_RECORD_CNT : M_DB_EMPTY, count);
    return count;
}
